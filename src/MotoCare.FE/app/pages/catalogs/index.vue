  ...
  <div class="page">
    ...
    <section class="card">
      <div class="table-wrap">
        <table v-if="activeTab === 'vehicleBrands'" class="data-table"><thead><tr><th>Mã</th><th>Tên hãng</th><th>Quốc gia</th><th>Trạng thái</th><th class="text-right">Thao tác</th></tr></thead><tbody><tr v-for="item in vehicleBrands" :key="item.id"><td class="mono">{{ item.code }}</td><td><AppEntityLink class="cell-main" :to="entityDetailRoute('VehicleBrand', item.id)">{{ item.name }}</AppEntityLink></td><td>{{ item.country || '—' }}</td><td><AppBadge :tone="item.isActive ? 'success' : 'neutral'">{{ item.isActive ? 'Hoạt động' : 'Tạm khóa' }}</AppBadge></td><td class="text-right"><button class="btn btn-secondary btn-sm" @click="openForm(item)">Chỉnh sửa</button></td></tr></tbody></table>
        <table v-else-if="activeTab === 'vehicleModels'" class="data-table"><thead><tr><th>Mã</th><th>Dòng xe</th><th>Hãng</th><th>Loại / phân khối</th><th>Trạng thái</th><th class="text-right">Thao tác</th></tr></thead><tbody><tr v-for="item in vehicleModels" :key="item.id"><td class="mono">{{ item.code }}</td><td><AppEntityLink class="cell-main" :to="entityDetailRoute('VehicleModel', item.id)">{{ item.name }}</AppEntityLink></td><td><AppEntityLink :to="entityDetailRoute('VehicleBrand', item.brandId)">{{ vehicleBrands.find(x => x.id === item.brandId)?.name || '—' }}</AppEntityLink></td><td>{{ item.vehicleType || '—' }} · {{ item.engineCapacityCc ? `${item.engineCapacityCc}cc` : '—' }}</td><td><AppBadge :tone="item.isActive ? 'success' : 'neutral'">{{ item.isActive ? 'Hoạt động' : 'Tạm khóa' }}</AppBadge></td><td class="text-right"><button class="btn btn-secondary btn-sm" @click="openForm(item)">Chỉnh sửa</button></td></tr></tbody></table>
        <table v-else-if="activeTab === 'partBrands'" class="data-table"><thead><tr><th>Mã</th><th>Hãng phụ tùng</th><th>Quốc gia</th><th>Liên hệ</th><th>Trạng thái</th><th class="text-right">Thao tác</th></tr></thead><tbody><tr v-for="item in partBrands" :key="item.id"><td class="mono">{{ item.code }}</td><td><AppEntityLink class="cell-main" :to="entityDetailRoute('PartBrand', item.id)">{{ item.name }}</AppEntityLink></td><td>{{ item.country || '—' }}</td><td>{{ item.contactInfo || '—' }}</td><td><AppBadge :tone="item.isActive ? 'success' : 'neutral'">{{ item.isActive ? 'Hoạt động' : 'Tạm khóa' }}</AppBadge></td><td class="text-right"><button class="btn btn-secondary btn-sm" @click="openForm(item)">Chỉnh sửa</button></td></tr></tbody></table>
        <table v-else-if="activeTab === 'partCategories'" class="data-table"><thead><tr><th>Mã</th><th>Danh mục phụ tùng</th><th>Thông số kỹ thuật</th><th>Mô tả</th><th>Trạng thái</th><th class="text-right">Thao tác</th></tr></thead><tbody><tr v-for="item in partCategories" :key="item.id"><td class="mono">{{ item.code }}</td><td><AppEntityLink class="cell-main" :to="entityDetailRoute('PartCategory', item.id)">{{ item.name }}</AppEntityLink></td><td><span v-if="item.specificationDefinitions?.length">{{ item.specificationDefinitions.map(x => `${x.name} · ${specificationTypeLabel(x.dataType || 'Text')}${x.unit ? ` (${x.unit})` : ''}`).join(', ') }}</span><span v-else>—</span></td><td>{{ item.description || '—' }}</td><td><AppBadge :tone="item.isActive ? 'success' : 'neutral'">{{ item.isActive ? 'Hoạt động' : 'Tạm khóa' }}</AppBadge></td><td class="text-right"><button class="btn btn-secondary btn-sm" @click="openForm(item)">Chỉnh sửa</button></td></tr></tbody></table>
        <table v-else class="data-table"><thead><tr><th>Mã</th><th>Dịch vụ</th><th class="text-right">Giá mặc định</th><th>Mô tả</th><th>Trạng thái</th><th v-if="isAdmin" class="text-right">Thao tác</th></tr></thead><tbody><tr v-for="item in serviceCategories" :key="item.id"><td class="mono">{{ item.code }}</td><td><AppEntityLink class="cell-main" :to="entityDetailRoute('ServiceCategory', item.id)">{{ item.name }}</AppEntityLink></td><td class="text-right">{{ formatCurrency(item.defaultPrice) }}</td><td>{{ item.description || '—' }}</td><td><AppBadge :tone="item.isActive ? 'success' : 'neutral'">{{ item.isActive ? 'Hoạt động' : 'Tạm khóa' }}</AppBadge></td><td v-if="isAdmin" class="text-right"><button class="btn btn-secondary btn-sm" @click="openForm(item)">Chỉnh sửa</button></td></tr></tbody></table>
      </div>
    </section>
