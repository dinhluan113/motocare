...
const disabledLocationSet = computed(() => new Set(props.disabledLocationIds))
const pendingDisabled = computed(() => Boolean(pendingValue.value && disabledLocationSet.value.has(pendingValue.value)))
...
<template>
  ...
  <AppModal :open="open" :title="title" :description="description" width="1040px" @close="open = false">
      ...
      <article v-for="rack in racks" :key="rack.rack" class="picker-rack">
        ...
        <div class="picker-frame">
          <div v-for="level in rack.levels" :key="level.level" class="picker-level">
            ...
            <button v-for="location in level.bins" :key="location.id" type="button" class="picker-bin" :class="{ selected: pendingValue === location.id, disabled: disabledLocationSet.has(location.id) }" :disabled="disabledLocationSet.has(location.id)" @click="pendingValue = location.id">
    ...
    <template #footer>
      ...
      <button type="button" class="btn btn-primary" :disabled="!pendingValue || pendingDisabled" @click="confirm"><MapPin :size="15" /> Chọn vị trí</button>
    </template>
  </AppModal>
</template>
